...
// ==========================================================================
// 🎮 2. LÓGICA DEL JUEGO Y VARIABLES GLOBALES
// ==========================================================================
...
function cambiarNombre() { 
    pedirNombre(); 
    if(nombreJugador !== "Desconocido") alert("Nombre actualizado a: " + nombreJugador); 
}
...
function escucharAutorizacionVIP() {
    ...
    db.collection("pases_vip").doc(nombreJugador).onSnapshot((doc) => {
        if (doc.exists && doc.data().autorizado === true) {
            ...
            guardarPartida(); subirPuntuacion();
...
function verificarEstadoVIPEnNube() {
    ...
    db.collection("pases_vip").doc(nombreJugador).get().then((doc) => {
        if (doc.exists && doc.data().autorizado === true) {
            ...
            casinoVIP = false; localStorage.setItem('casinoVIP', 'false');
        }
        guardarPartida();
    }).catch(() => {});
}
    ...
    if (confirm("⚠️ ¿ERES EL CAMARERO?\n\nVerificando autenticidad en la base de datos...")) {
        ...
        db.runTransaction((transaction) => {
            return transaction.get(docRef).then((doc) => {
                if (!doc.exists) throw "CUPON_FALSO";
                if (!doc.data().activo) throw "CUPON_YA_CANJEADO";
                transaction.update(docRef, {
                    activo: false, canjeadoEn: firebase.firestore.FieldValue.serverTimestamp()
                });
                return true;
            });
        }).then(() => {
            inventarioCupones.splice(cuponActivoIndex, 1);
            guardarPartida(); cuponActivoIndex = -1; clearInterval(intervaloRelojCupon);
...
function abrirLogros() { 
    ocultarTodosModales(); 
    const modalLogros = document.getElementById('logros-modal');
    if(modalLogros) modalLogros.classList.remove('oculto'); 
    
    const contenedor = document.getElementById('lista-logros-contenedor'); 
...
function abrirMenuPrincipal() { pausarJuego(); ocultarTodosModales(); const m = document.getElementById('menu-modal'); if(m) m.classList.remove('oculto'); }
function abrirJuerguistas() { ocultarTodosModales(); const m = document.getElementById('juerguistas-modal'); if(m) m.classList.remove('oculto'); renderizarJuerguistas(); }
function abrirOpciones() { ocultarTodosModales(); const m = document.getElementById('opciones-modal'); if(m) m.classList.remove('oculto'); }
function abrirManualDirecto() { ocultarTodosModales(); const m = document.getElementById('manual-modal'); if(m) m.classList.remove('oculto'); }
function volverAlMenu() { ocultarTodosModales(); const m = document.getElementById('menu-modal'); if(m) m.classList.remove('oculto'); }
...
function abrirInventarioCupones() {
    ocultarTodosModales(); 
    const m = document.getElementById('cupones-inventario-modal');
    if(m) m.classList.remove('oculto');
    ...
    if (!inventarioCupones || inventarioCupones.length === 0) {
        contenedor.innerHTML = `<div style="padding: 30px 10px;"><span style="font-size: 40px;">🕸️</span><p style="color:#666; margin-top: 15px; font-weight:bold; font-size:12px; line-height:1.6;">No tienes cupones todavía...<br><br>¡Prueba suerte abriendo Sobres VIP o pásate el juego para ganar!</p></div>`; 
        return;
    }
